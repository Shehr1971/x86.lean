...
| .assign v .read =>
  [.callq ("read_int") 0, .movq %rax (.var v)]
...
| .ret (.plus a b) =>  [.movq (atom a) %rax, .addq (atom b) %rax, .jmp "fini"]
| .ret (.minus a b) =>  [.movq (atom a) %rax, .subq (atom b) %rax, .jmp "fini"]
| .ret (.negative a) =>  [.movq (atom a) %rax, .negq %rax, .jmp "fini"]
| .ret (.atom a) => [.movq (atom a) %rax, .jmp "fini"]
